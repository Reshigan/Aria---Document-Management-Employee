...
const OperationsBasics: React.FC = () => {
  const [modules, setModules] = useState<Module[]>([
    { id: '1', title: 'Introduction to Operations', duration: '20 min', completed: true, locked: false },
    { id: '2', title: 'Inventory Management Basics', duration: '30 min', completed: false, locked: false },
    { id: '3', title: 'Purchase Order Processing', duration: '25 min', completed: false, locked: true },
    { id: '4', title: 'Goods Receipt & Dispatch', duration: '25 min', completed: false, locked: true },
    { id: '5', title: 'Warehouse Management', duration: '30 min', completed: false, locked: true },
    { id: '6', title: 'Operations Assessment', duration: '20 min', completed: false, locked: true },
  ]);
  ...
  return (
    <Box sx={{ p: 3, background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%)', minHeight: '100vh' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4" sx={{ color: 'white', fontWeight: 700 }}>Operations Basics Training</Typography>
        <Chip label="Beginner" color="success" />
      </Box>
      <Grid container spacing={3} sx={{ mb: 3 }}>
        {[{ label: 'Total Modules', value: modules.length, icon: <School />, color: '#667eea' }, { label: 'Completed', value: completedCount, icon: <CheckCircle />, color: '#4CAF50' }, { label: 'Progress', value: `${Math.round(progress)}%`, icon: <Assignment />, color: '#FF9800' }, { label: 'Est. Time', value: '2.5 hrs', icon: <Timer />, color: '#E91E63' }].map((stat, index) => (
...
export default OperationsBasics;
