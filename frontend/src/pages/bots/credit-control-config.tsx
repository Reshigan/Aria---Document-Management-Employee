/**
 * Credit Control Agent - Configuration Page
...
export default function CreditControlConfig() {
  ...
  const loadConfiguration = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/agents/credit_control/config`, {
  ...
  const handleExecute = async () => {
    try {
      const API_BASE = import.meta.env.VITE_API_URL || 'https://aria-api.reshigan-085.workers.dev';
      const response = await fetch(`${API_BASE}/api/agents/credit_control/execute`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item>
          <Chip label="Value: R90K-R180K/year saved" color="success" />
        </Grid>
      </Grid>
