...
export default function CalendarOffice365Config() {
  ...
  const loadConfiguration = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE}/api/agents/calendar_office365/config`, {
  ...
  const handleExecute = async () => {
    try {
      const API_BASE = import.meta.env.VITE_API_URL || 'https://aria-api.reshigan-085.workers.dev';
      const response = await fetch(`${API_BASE}/api/agents/calendar_office365/execute`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Grid container spacing={2} sx={{ mb: 3 }}>
        ...
        <Grid item>
          <Chip label="Value: R30K-R60K/year saved" color="success" />
        </Grid>
      </Grid>
