    ...
    <div className="bg-gradient-to-br from-gray-50 to-indigo-50 dark:from-gray-900 dark:to-gray-800 container mx-auto p-4">
      ...
      {activeTab === 'security' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 space-y-3">
          <div>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input type="number" min="6" max="32" value={securitySettings.password_min_length} onChange={(e) => setSecuritySettings({...securitySettings, password_min_length: parseInt(e.target.value)})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
              </div>
            </div>
          </div>
          <div className="border-t pt-6">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <input type="number" min="5" max="480" value={securitySettings.session_timeout_minutes} onChange={(e) => setSecuritySettings({...securitySettings, session_timeout_minutes: parseInt(e.target.value)})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Max Login Attempts</label>
                <input type="number" min="3" max="10" value={securitySettings.max_login_attempts} onChange={(e) => setSecuritySettings({...securitySettings, max_login_attempts: parseInt(e.target.value)})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
              </div>
            </div>
          </div>
          <div className="border-t pt-6">
            ...
            {securitySettings.ip_whitelist_enabled && (
              <div>
                <div className="flex gap-2 mb-3">
                  <input type="text" value={newIP} onChange={(e) => setNewIP(e.target.value)} placeholder="Enter IP address" className="flex-1 px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
                  <Button onClick={() => { if (newIP.trim()) { setSecuritySettings({...securitySettings, ip_whitelist: [...securitySettings.ip_whitelist, newIP.trim()]}); setNewIP(''); }}} className="bg-blue-600 dark:bg-blue-500 text-white">
                ...
                <div className="space-y-2">
                  {securitySettings.ip_whitelist.map((ip, idx) => (
                    <div key={idx} className="flex items-center justify-between bg-gray-50 dark:bg-gray-900 p-3 rounded">
                      ...
                      <button onClick={() => setSecuritySettings({...securitySettings, ip_whitelist: securitySettings.ip_whitelist.filter(i => i !== ip)})} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:text-red-300">
          ...
          <div className="flex justify-end">
            <Button onClick={handleSaveSecuritySettings} disabled={saving} className="bg-blue-600 dark:bg-blue-500 text-white">
      ...
      {activeTab === 'notifications' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 space-y-3">
          ...
          <div className="flex justify-end">
            <Button onClick={handleSaveNotificationSettings} disabled={saving} className="bg-blue-600 dark:bg-blue-500 text-white">
      ...
      {activeTab === 'backup' && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-4 space-y-3">
            ...
            {backupSettings.auto_backup_enabled && (
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={backupSettings.backup_frequency} onChange={(e) => setBackupSettings({...backupSettings, backup_frequency: e.target.value})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md">
                ...
                <div>
                  ...
                  <input type="time" value={backupSettings.backup_time} onChange={(e) => setBackupSettings({...backupSettings, backup_time: e.target.value})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Retention Period (days)</label>
                  <input type="number" min="1" max="365" value={backupSettings.retention_days} onChange={(e) => setBackupSettings({...backupSettings, retention_days: parseInt(e.target.value)})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Backup Location</label>
                  <input type="text" value={backupSettings.backup_location} onChange={(e) => setBackupSettings({...backupSettings, backup_location: e.target.value})} className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" placeholder="s3://bucket-name" />
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end">
            <Button onClick={handleSaveBackupSettings} disabled={saving} className="bg-blue-600 dark:bg-blue-500 text-white">
              ...
              {apiKeys.map((key) => (
                <div key={key.id} className="border rounded-lg p-4 flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <code className="text-sm bg-gray-100 dark:bg-gray-700 px-3 py-1 rounded font-mono">{key.key.substring(0, 20)}...</code>
                  </div>
                  <button onClick={() => handleDeleteAPIKey(key.id)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:text-red-300">
                ...
                {copiedKey ? (
                  <div className="space-y-4">
                    <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 rounded-lg p-4">
                      ...
                      <p className="text-sm text-yellow-700 dark:text-yellow-300 mt-1">You won't be able to see it again.</p>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">API Key</label>
                      <code className="block text-sm bg-gray-100 dark:bg-gray-700 px-3 py-2 rounded font-mono break-all">{copiedKey}</code>
                    </div>
                    <Button onClick={() => { setShowNewKeyModal(false); setCopiedKey(''); }} className="bg-blue-600 dark:bg-blue-500 text-white w-full">Done</Button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Key Name *</label>
                      <input type="text" value={newKeyName} onChange={(e) => setNewKeyName(e.target.value)} placeholder="e.g., Production API" className="w-full px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md" />
                    </div>
                    <div className="flex justify-end gap-3">
                      <Button variant="outline" onClick={() => { setShowNewKeyModal(false); setNewKeyName(''); }}>Cancel</Button>
                      <Button onClick={handleCreateAPIKey} disabled={!newKeyName.trim()} className="bg-blue-600 dark:bg-blue-500 text-white">Create Key</Button>
                    </div>
                  </div>
                )}
              </div>
